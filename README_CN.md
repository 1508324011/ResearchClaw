...
- **Web 运行时已具备第一阶段能力**：文献库、浏览器导入工作区（arXiv / DOI / URL / PDF）、搜索、浏览器任务恢复页、论文概览页、由服务端本地 PDF 支撑的阅读页、笔记页。
...
- Phase 1 的浏览器工作流刻意保持精简：显式选择标识符类型（`arXiv`、`DOI`、`URL`）、直接上传 PDF、先进入 `/papers/:paperId` 概览页、用 `/jobs` 恢复进行中的任务，再衔接到由服务端本地 PDF 支撑的阅读页，以及笔记页/来源链接。
