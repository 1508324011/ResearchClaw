...
- **Web 运行时已具备第一阶段能力**：文献库、浏览器导入工作区（arXiv / DOI / URL / PDF）、搜索、论文概览页、阅读页、笔记页。
...
| **Web 阅读页**   | 浏览器原生论文摘要与最新笔记预览，接口为 `GET /reading/:paperId`                |
...
- Phase 1 的浏览器工作流刻意保持精简：显式选择标识符类型（`arXiv`、`DOI`、`URL`）、直接上传 PDF、先进入 `/papers/:paperId` 概览页，再衔接到阅读页/笔记页/来源链接。
