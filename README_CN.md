...
<p align="center">
  ResearchClaw 保留原始 Electron 桌面应用，同时新增了一个 server-first、Docker-first 的浏览器运行时，用于导入、文献库、搜索、任务恢复、阅读和笔记。
</p>
...
- **Web 运行时已具备第一阶段能力**：文献库、浏览器导入工作区（arXiv / 严格完整 DOI / URL / 25 MB 内 PDF）、搜索、浏览器任务恢复页、论文概览页、由服务端本地 PDF 支撑的阅读页、笔记页。
...
| **Web 导入**     | 浏览器原生导入工作区，支持 arXiv / 严格完整 DOI / URL 标识符与 25 MB 内直接 PDF 上传              |
| **Web 概览页**   | 浏览器原生论文概览页 `/papers/:paperId`，展示元数据、摘要以及阅读/笔记/来源入口                   |
| **Web 搜索**     | 同源文本/语义搜索，接口为 `GET /search`                                                           |
| **Web 任务页**   | 浏览器原生任务恢复页 `/jobs`，支持从服务端快照恢复当前任务并继续接收 SSE 进度                     |
| **Web 阅读页**   | 浏览器原生、由服务端本地 PDF 支撑的阅读视图与固定侧边最新笔记预览，接口为 `GET /reading/:paperId` |
...
- Phase 1 的浏览器工作流刻意保持精简：显式选择标识符类型（`arXiv`、严格完整 `DOI`、`URL`）、直接上传 25 MB 内 PDF、先进入 `/papers/:paperId` 概览页、用 `/jobs` 恢复进行中的任务，再衔接到由服务端本地 PDF 支撑且带固定侧边笔记摘要的阅读页，以及笔记页/来源链接。
